...
def add_cp_s(x, cp_len):
    return torch.cat((x[...,-cp_len:,:], x), dim=-2)

def add_cp_m(x, cp_len):
    return torch.cat((x[...,-cp_len:], x), dim=-1)

def rm_cp_m(x, cp_len):
    return x[...,cp_len:]
def rm_cp_s(x, cp_len):
    return x[...,cp_len:,:]
